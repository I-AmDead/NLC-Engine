...
XRCORE_API const xr_string& __cdecl xra_sprintf(LPCSTR format, va_list &mark)
{
...
XRCORE_API const xr_string& __cdecl xra_sprintf(LPCSTR format, ...) // uniform sprintf
{
	va_list mark;	
	va_start(mark, format);					
	return xra_sprintf(format, mark);							
}
...
extern "C"
{
	...
	{		
		va_list mark;	
		va_start(mark, format);					
		return xra_sprintf(format, mark).c_str();
	}
};
...
{
	Log(xra_sprintf(format, mark).c_str());	
}
...
extern "C"
{
	XRCORE_API void __stdcall  xr_MsgCB(LPCSTR format, ...) // alternate MsgCB version
	{
		va_list mark;
		va_start(mark, format);
		ExecLogCB (xra_sprintf(format, mark).c_str());
	}
