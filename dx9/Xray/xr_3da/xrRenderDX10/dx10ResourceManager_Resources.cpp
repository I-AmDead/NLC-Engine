...
SVS*	CResourceManager::_CreateVS		(LPCSTR _name)
{
	...
	else
	{
		...
		{
			string1024			tmp;
			sprintf_s			(tmp, "DX10: %s is missing. Replace with stub_default.vs", cname);
			Msg					(tmp);
...
SPS*	CResourceManager::_CreatePS			(LPCSTR _name)
{
	...
	else
	{
		...
		if (!R)
		{
			...
			//Memory.mem_compact();
			sprintf_s				(tmp, "DX10: %s is missing. Replace with stub_default.ps", cname);
			Msg					(tmp);
...
SGS*	CResourceManager::_CreateGS			(LPCSTR name)
{
	...
	else
	{
		...
		if (!R)
		{
			...
			//	TODO: HACK: Test failure
			//Memory.mem_compact();
			sprintf_s				(tmp, "DX10: %s is missing. Replace with stub_default.gs", cname);
			Msg					(tmp);
