						...
						bool need_result = false)
{
	...
	if (classic) // вызов глобальной функции
	{
		...
		if (ret->error)
		{			
			g_debug_context += xra_sprintf("#FAIL: LuaExecute returned error %d for function '%s': %s\n", ret->error, func_name, ret->c_str("(null)"));
			return false;
		}
	...
	if (0 == err)
	{
		...
		if (0 == err)
		{
			...
			g_debug_context += xra_sprintf("lua_pcall failed with code '%s', err = %d \n", code, err);
	...
	else
	{
		...
		g_debug_context += xra_sprintf("luaL_loadbuffer failed with code '%s', err = %d: %s \n", code, err, lua_tostring(L, -1));
		lua_settop(L, save_top);
		return false;
	}
