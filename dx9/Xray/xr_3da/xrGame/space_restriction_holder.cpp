...
{
	return  !bridge->shape() && !bridge->m_ref_count;
}

void UnregisterBridge(CSpaceRestrictionBridge *bridge)
{
	u32 _ptr = (u32)bridge;
	auto it = g_bridges.find(_ptr);
	if (it == g_bridges.end())
...
void CSpaceRestrictionHolder::unregister_restrictor			(CSpaceRestrictor *space_restrictor)
{
	...
	if ((*I).second)
	{
		if ( UnusedBridge ((*I).second) )
			DestroyBridge ((*I).second);
		(*I).second = NULL;
	}
