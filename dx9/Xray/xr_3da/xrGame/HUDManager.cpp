...
{
//gnzi	if (!psHUD_Flags.is(HUD_DRAW_RT2))	
//--		return false;

//gnzi	if (!psHUD_Flags.is(HUD_WEAPON|HUD_WEAPON_RT|HUD_WEAPON_RT2))return false;
	if (!psHUD_Flags.is(HUD_WEAPON|HUD_WEAPON_RT))return false;

	if(!need_render_hud())			return false;

	return (pHudItem && pHudItem->render_item_ui_query() );
}

void   CHUDManager::RenderActiveItemUI()
{
//	if (!psHUD_Flags.is(HUD_DRAW_RT2))
	if (!psHUD_Flags.is(HUD_DRAW_RT))
		return;

	pHudItem->render_item_ui		();
}
*/
