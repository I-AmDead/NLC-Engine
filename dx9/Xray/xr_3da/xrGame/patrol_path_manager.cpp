...
void CPatrolPathManager::select_point(const Fvector &position, u32 &dest_vertex_id)
{
	...
	if (!actual() || !m_path->vertex(m_curr_point_index)) {
		...
		{
			Msg("!#ERROR: any vertex in patrol path [%s] in inaccessible for object [%s]. Trying switch to offline",	*m_path_name, *m_game_object->cName());
			m_actuality			= true;
			m_completed			= false;
			dest_vertex_id		= m_game_object->level_vertex_id();			
