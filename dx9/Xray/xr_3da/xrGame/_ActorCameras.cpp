...
//gnzi void	collide_camera( CCameraBase & camera, float _viewport_near  );
...
void CActor::cam_Update(float dt, float fFOV)
{
	...
	float HeightInterpolationSpeed = 5.f;
	...
	//gnzi if (Level().CurrentEntity() == this)
...
void CActor::update_camera (CCameraShotEffector* effector)
{
	...
	effector->ApplyDeltaAngles( &(pACam->pitch), &(pACam->yaw) );
