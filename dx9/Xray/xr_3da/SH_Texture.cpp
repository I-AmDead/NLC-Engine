...
CTexture::~CTexture()
{
	__try
	{
		LPCSTR szName = *cName;
		if (szName)
			MsgCB("$#CONTEXT: ~CTexture for %s ", szName);
		Unload();
	...
	{
		Log("!#EXCEPTION: catched in CTexture::~CTexture()");
	}
}
...
//.	if (flags.bLoaded)		Msg		("* Unloaded: %s",cName.c_str());
	...
	if (pSurface)
	{
		int refCount = pSurface->Release(); pSurface = 0;
		if (refCount > 1) 
			Msg("*#LEAK_CHECK: Unloaded texture %-45s refCount = %d, size = %d K ", cName.c_str(), refCount, flags.MemoryUsage / 1024);
	}
