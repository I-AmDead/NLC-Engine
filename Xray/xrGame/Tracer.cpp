...
IC void FillSprite_Circle      (FVF::LIT*& pv, const Fvector& pos, const float width, const float length, u32 color)
{
	...
	//	Tri 1
	pv->set         (d.x+pos.x, d.y+pos.y, d.z+pos.z, color, t_crcl.min.x, t_crcl.max.y); pv++;		// 0.f,1.f);
	pv->set         (a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_crcl.min.x, t_crcl.min.y); pv++;		// 0.f,0.f);
	pv->set         (c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_crcl.max.x, t_crcl.max.y); pv++;		// 1.f,1.f);
	//	Tri 2																					
	pv->set         (c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_crcl.max.x, t_crcl.max.y); pv++;	// 1.f,1.f);
	pv->set         (a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_crcl.min.x, t_crcl.min.y); pv++;	// 0.f,0.f);
	pv->set         (b.x+pos.x, b.y+pos.y, b.z+pos.z, color, t_crcl.max.x, t_crcl.min.y); pv++;	// 1.f,0.f);
...
IC void FillSprite_Line	(FVF::LIT*& pv, const Fvector& pos, const Fvector& dir, const float width, const float length, u32 color)
{
    const Fvector& T	= dir;
    Fvector				R;      
	...
	//	TODO: return code back to indexed rendering since we use quads
	//	Tri 1
	pv->set         (d.x+pos.x, d.y+pos.y, d.z+pos.z, color, t_tracer.min.x, t_tracer.max.y); pv++;
	pv->set         (a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_tracer.min.x, t_tracer.min.y); pv++;
	pv->set         (c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_tracer.max.x, t_tracer.max.y); pv++;
	//	Tri 2																					
	pv->set         (c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_tracer.max.x, t_tracer.max.y); pv++;
	pv->set         (a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_tracer.min.x, t_tracer.min.y); pv++;
	pv->set         (b.x+pos.x, b.y+pos.y, b.z+pos.z, color, t_tracer.max.x, t_tracer.min.y); pv++;
...
void  CTracer::Render	(FVF::LIT*&verts, const Fvector& pos,
...
//			float f_distance	= Device.vCameraPosition.distance_to(pos);
			...
			FillSprite_Circle	(verts, pos,	k_speed*width*m_circle_size_k,	k_speed*width*m_circle_size_k, m_aColors[colorID]);
		}
