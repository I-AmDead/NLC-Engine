...
bool can_capture( CPHCharacter   *a_character, CPhysicsShellHolder	*a_taget_object  )
{
	...
	CKinematics* p_kinematics = smart_cast<CKinematics*>( a_character->PhysicsRefObject()->Visual( ) );
	VERIFY( p_kinematics );
...
bool can_capture( CPHCharacter   *a_character, CPhysicsShellHolder	*a_taget_object, u16 a_taget_element  ) 
{
	...
	CKinematics* K=	smart_cast<CKinematics*>( a_taget_object->Visual( ) );
...

{
	...
	CKinematics* K=	smart_cast<CKinematics*>( m_taget_object->Visual( ) );
	VERIFY( K );
...
void CPHCapture::Init( )
{
	...
	CKinematics* p_kinematics = smart_cast<CKinematics*>( m_character->PhysicsRefObject()->Visual( ) );
	VERIFY( p_kinematics );
	CInifile	* ini = p_kinematics->LL_UserData();
	VERIFY( ini );
