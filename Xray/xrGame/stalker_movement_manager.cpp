...
void CStalkerMovementManager::cleanup_after_animation_selector	()
{
...
void CStalkerMovementManager::set_nearest_accessible_position(Fvector desired_position, u32 level_vertex_id)
{
	...
	if (!restrictions().accessible(desired_position)) {
		...
		R_ASSERT					(restrictions().accessible(level_vertex_id));
		R_ASSERT					(restrictions().accessible(desired_position));
	}
	else {
	...
	R_ASSERT						(ai().level_graph().inside(level_vertex_id,desired_position));

	R_ASSERT						(restrictions().accessible(level_vertex_id) || show_restrictions(&restrictions()),*object().cName());
	set_level_dest_vertex		(level_vertex_id);
	
	R_ASSERT						(restrictions().accessible(desired_position) || show_restrictions(&restrictions()),*object().cName());
...
void CStalkerMovementManager::remove_links			(CObject *object)
{
	m_last_query_object					= 0;
}