...
#include "../Include/xrRender/SkeletonCustom.h"
...
void DBG_DrawPHAbstruct(SPHDBGDrawAbsract* a)
{
	...
	{
		if(ph_world->Processing()) dbg_ph_draw_mode = dmCashedSecondary;
...
void DBG_AnimBlend(CKinematicsAnimated &ka, const CBlend &B )
{
...
void DBG_AnimPartState( CKinematicsAnimated &ka, u16 part )
{
...
void DBG_AnimState( CKinematicsAnimated &ka )
{
...
void	DBG_DrawBones( const Fmatrix &xform,  CKinematics *K )
{
...
{
	CKinematics *K = smart_cast<CKinematics*> ( O.Visual() );
...
{
	return trace_object && static_cast<CObject*>(obj->ref_object()) == trace_object;
}
...
void DBG_ObjBeforeStep			( CPHObject *obj )
{
	if( is_trace_obj(obj) )
	{
		...
		if(obj->ref_object()->PPhysicsShell())
		{
			obj->ref_object()->PPhysicsShell()->dbg_draw_force		( 0.1f, D3DCOLOR_XRGB( 0, 0, 255 ) ); 
			Fmatrix form = Fidentity;
			obj->ref_object()->PPhysicsShell()->GetGlobalTransformDynamic( &form );
...
void DBG_ObjAfterStep			( CPHObject *obj )
{
	if( is_trace_obj(obj) )
	{
		...
		if(obj->ref_object()->PPhysicsShell())
		{
			obj->ref_object()->PPhysicsShell()->dbg_draw_velocity	( 0.1f, D3DCOLOR_XRGB( 255, 0, 0 ) );
		}
