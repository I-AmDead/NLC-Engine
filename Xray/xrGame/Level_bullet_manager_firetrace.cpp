...
void CBulletManager::FireShotmark (SBullet* bullet, const Fvector& vDir, const Fvector &vEnd, collide::rq_result& R, u16 target_material, const Fvector& vNormal, bool ShowMark)
{
	...
	else 
	{
		...
		{
			//добавить отметку на материале
			::Render->add_StaticWallmark	(mtl_pair->CollideMarks[::Random.randI(0,mtl_pair->CollideMarks.size())], vEnd, bullet->wallmark_size, pTri, pVerts);
		}
	}
...
//	Fvector			hit_normal;
	...
	if (E.bullet.flags.allow_sendhit && !E.Repeated)
	{
		...
		if (GameID() != GAME_SINGLE && E.bullet.flags.allow_sendhit && smart_cast<CActor*>(E.R.O)
