...
//	{
		...
		MsgCB("CScriptGameObject::add_restrictions('%s', '%s') for '%s' ", out, in, monster->Name_script());
//	}
...
CScriptGameObject *CScriptGameObject::item_in_slot	(u32 slot_id) const
{
	...
	if (inventory_owner->inventory().m_slots.size() <= slot_id) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : invalid slot id for class member item_in_slot : %d, m_slots.size() : %d!", slot_id, inventory_owner->inventory().m_slots.size());
		return		(0);
	}
...
#include "artifact.h"
void CScriptGameObject::set_buy_item_condition_factor(float factor)
{
	CInventoryOwner	*pInventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!pInventoryOwner) {
	...
	CArtefact *pArtefact = smart_cast<CArtefact*>(&object());
	if (!pArtefact)
	{
...
void CScriptGameObject::set_buy_artefact_condition_factor(float factor)
{
	CInventoryOwner	*pInventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!pInventoryOwner) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CInventoryOwner : cannot access class member buy_artefact_condition_factor!");
		return;
	};
	CArtefact *pArtefact = smart_cast<CArtefact*>(&object());
	if (pArtefact)
	{
		clamp(factor, 0.f, 1.0f);
		pInventoryOwner->trade_parameters().buy_artefact_condition_factor = factor;
	}
}
