...
public:
				void		Create				( u16 id, CKinematicsAnimated	*K, bool collide_ );	
				void		Destroy				( );
public:
...
IC		CKinematics			*Kinematics			( )									const { return m_foot.Kinematics(); }
IC CKinematicsAnimated		*KinematicsAnimated	( )									const { return m_K; }
IC				u16			ref_bone			( )									const { return m_foot.ref_bone();	}
				...
				u16			foot_matrix_predict ( Fmatrix& foot, Fmatrix& toe, float time, CKinematicsAnimated *K ) const ;
#ifdef		DEBUG
