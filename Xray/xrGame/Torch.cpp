...
void CTorch::Load(LPCSTR section) 
{
	...
	if (pSettings->line_exist(section, "snd_torch_on"))
		m_sounds.LoadSound(section, "snd_torch_on", "m_OnSnd", true, SOUND_TYPE_ITEM_USING);
	if (pSettings->line_exist(section, "snd_torch_off"))
		m_sounds.LoadSound(section, "snd_torch_off", "m_OffSnd", true, SOUND_TYPE_ITEM_USING);
#endif
}
...
void CTorch::Switch	(bool light_on)
{
	...
	if (pA){// озвучка акторского фонарика
