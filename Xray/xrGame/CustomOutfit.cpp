...
void CCustomOutfit::Load(LPCSTR section) 
{
	...
	for (int i = ALife::eHitTypeBurn; i < ALife::eHitTypeMax; i++)
	{
		...
		//if (coef > 1.f)
			clamp<float> (value, 0.f, 0.8f);  // абсолютной защиты не бывает в реальности
...
void CCustomOutfit::Hit(SHit* pHDS)
{
	...
	if (it != full_list.end())
	{
		...
		BP.condition = cond;
		if (actor_outfit)
			Msg("# outfit hit received, bone = %d, hit_power = %.5f, real hit = %.4f, condition = %.5f ", element, hit_power, hp, cond);		
	}
	else
		HitSpread(hit_power);
...
float CCustomOutfit::GetHitTypeProtection(SHit *pHDS)
{
	...
	if (m_bActorOutfit)
		Msg("# element = %2d, base = %.3f, bone = %.3f,  hit ratio = %.3f ", element, fBase, bone, result);
...
float	CCustomOutfit::HitThruArmour(SHit *pHDS)
{
...
#endif
	float BoneArmour = m_boneProtection->getBoneArmour(element) * ( 1 - pHDS->armor_piercing );	
	...
	if (Actor()->GetOutfit() == this)
		Msg("# hit_power = %7.3f, element = %2d, BoneArmor = %.3f, result = %.3f ", hit_power, element, BoneArmour, NewHitPower);
...
void CCustomOutfit::SetCondition(float fNewCondition) // TODO: лечит/калечит элементы защиты в порядке перебора костей!
{
	...
	else
	{
		...
		Msg("@#DBG: COutfit::SetCondition change = %.5f ", change);
		...
		for (auto it = list.begin(); it != list.end(); it++)
		{
			...
			BP.condition += delta;
			Msg("@  bone = %2d, delta = %.5f, condition = %.3f, change -= %.5f ", (int)it->first, delta, BP.condition, delta * BP.importance);
