...
#include "../xr_3da/common/doug_lea_memory_allocator.h"
...
enum E_COMMON_FLAGS{
	...
	flDeadBodyCollision = (1<<1),
	flVolumetricLightsForce = (1<<2),
};
Flags32 g_uCommonFlags = { flAiUseTorchDynamicLights | flDeadBodyCollision | flVolumetricLightsForce };
...
DLL_API void		stat_memory_impl()
{
	...
	Msg		("* [ D3D ]: textures[%d K], c_base[%d K], c_lmaps [%d K]", (m_base + m_lmaps) / 1024, c_base / 1024, c_lmaps / 1024);
	...
	if (1) // _crt_heap != _process_heap
		Msg		("* [x-ray]: crt heap[%d K], process heap[%d K], lib heap [%d K], game lua[%d K], engine lua[%d K], render[%d K]",
					_crt_heap/1024, _process_heap/1024, _lib_heap/1024, _game_lua/1024, _engine_lua/1024, _render/1024);
	else
...
//#endif // DEBUG
	...
	g_uCommonFlags.set(flVolumetricLightsForce, FALSE);
	CMD3(CCC_Mask, "volumetric_lights_super", &g_uCommonFlags, flVolumetricLightsForce);
