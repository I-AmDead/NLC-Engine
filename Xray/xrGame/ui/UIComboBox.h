...
public:
	...
	CUIListBoxItem*		AddItem_				(LPCSTR str, int _data);
			...
			void		SetItem					(int i);
	virtual void		Init					(float x, float y, float width) { InitComboBox(Fvector2().set(x,y), width); }
	virtual void		Init					(float x, float y, float width, float height) { InitComboBox(Fvector2().set(x, y), width); }
