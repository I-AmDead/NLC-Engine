	...
	else
	{
		if(m_pCurrentDialog->IsWeSpeaking(m_pOurDialogManager))
		{
			...
			{			
				...
				{
					CPhrase* phrase = *it;
					AddQuestion(phrase->GetText(), phrase->GetID(), n);
				}
			}
			else
				UpdateQuestions();
...
void CUITalkWnd::AddQuestion(const shared_str& text, const shared_str& value, int n)
{
	if(text.size() == 0) return;
	UITalkDialogWnd->AddQuestion(*script_translate(text, value),value.c_str(), n);
}
