...
bool CUITradeWnd::CanMoveToOther(PIItem pItem)
{
	...
	if (pArtefact)
	{
		if (pItem->GetCondition() < m_pOthersInvOwner->trade_parameters().buy_artefact_condition_factor)
			return false;
	...
	{
		LPCSTR		name	= smart_cast<CObject*>(m_pOthersInvOwner)->Name_script();
		if (0 == xr_strcmp(name, "hos_junkman"))
			return true;

		LPCSTR		sect	= smart_cast<CObject*>(pItem)->cNameSect().c_str();
		if ( strstr(sect, "mutant_") && (0==xr_strcmp(name, "dsc_part_exchanger") || 0==xr_strcmp(name, "pahom")) )
			return true;
