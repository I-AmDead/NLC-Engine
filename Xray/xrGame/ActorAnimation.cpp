...
void SActorState::Create(CKinematicsAnimated* K, LPCSTR base)
{
	...
	if (0 == xr_strcmp(base, "norm"))
		m_torso_idle	= K->ID_Cycle(strconcat(sizeof(buf),buf,base,"_torso_0_idle_0"));
	else
		m_torso_idle	= K->ID_Cycle(strconcat(sizeof(buf),buf,base,"_torso_0_aim_0"));
...
void CActor::g_SetAnimation( u32 mstate_rl )
{
	...
	{	
		...
		{
			CHudItem* pHudItem = smart_cast<CHudItem*>(inventory().ActiveItem());	
			if (pHudItem) pHudItem->onMovementChanged(mcAnyMove);
		}
	};
	...
	if(!M_torso)
	{
		...
		if (_i == NULL)
		{
			if (!m_bAnimTorsoPlayed && !GamePersistent().IsInventoryMenu())
			{
				STorsoWpn* TW = &ST->m_torso[5 - 1];
