...
void CNightVisionDevice::LoadConfig(LPCSTR section)
{
	...
	{		
		m_sounds.LoadSound(section, "sound_on",     "m_OnSnd",			true,		SOUND_TYPE_ITEM_USING);
		m_sounds.LoadSound(section, "sound_off",    "m_OffSnd",			true,		SOUND_TYPE_ITEM_USING);
		m_sounds.LoadSound(section, "sound_idle",   "m_IdleSnd",		true,		SOUND_TYPE_ITEM_USING);
		m_sounds.LoadSound(section, "sound_broken", "m_BrokenSnd",		true,		SOUND_TYPE_ITEM_USING);
...
void CNightVisionDevice::SwitchNightVision(bool vision_on)
{
	...
	if (m_EffectorSect.size() && !b_allow) {
		m_sounds.PlaySound("m_BrokenSnd", pA->Position(), pA, bPlaySoundFirstPerson);
		return;
	}
	...
	if(m_bPowered) {
		...
		if(!pp) {
			if (m_EffectorSect.size())
			{
				...
				m_sounds.PlaySound("m_OnSnd", pA->Position(), pA, bPlaySoundFirstPerson);
				m_sounds.PlaySound("m_IdleSnd", pA->Position(), pA, bPlaySoundFirstPerson, true);
			}
		}
	} else {
 		CEffectorPP* pp = pA->Cameras().GetPPEffector((EEffectorPPType)effNightvision);
		if(pp){
			pp->Stop			(1.0f);
			m_sounds.PlaySound("m_OffSnd", pA->Position(), pA, bPlaySoundFirstPerson);
			m_sounds.StopSound("m_IdleSnd");
		}
	}
}
