...
u32 CScriptGameObject::get_crouch_factor		()
{
	CActor* actor = smart_cast<CActor*>(&object());
	if(actor){
		return actor->get_crouch_factor();
	}
	else
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "ScriptGameObject : attempt to call get_crouch_factor method for non-actor object");
		return NULL;
	}
}
...
CCar* CScriptGameObject::get_car	()
{
	...
	if (!car) {
		...
		log_script_error("cannot cast %s to CCar ", object().CppClassName());
#endif
