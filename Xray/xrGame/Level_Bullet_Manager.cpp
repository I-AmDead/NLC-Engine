				   ...
				   bool SendHit)
{
	...
	hit_param.power			= power          * cartridge.m_kHit;
	hit_param.impulse		= impulse        * cartridge.m_kImpulse;
	...
	armor_piercing			= cartridge.m_kAP;
...
{
	if ( air_resistance*(time + air_resistance_epsilon) <= 1.f ) {
...
BOOL CBulletManager::firetrace_callback	(collide::rq_result& result, LPVOID params)
{
	...
	float const	air_resistance		= (GameID() == GAME_SINGLE) ? Level().BulletManager().m_fAirResistanceK : bullet.air_resistance;
	...
	CKinematics* const kinematics	= smart_cast<CKinematics*>(result.O->Visual());
	if (!kinematics)
		return						(FALSE);
...
bool CBulletManager::process_bullet			(collide::rq_results & storage, SBullet& bullet, u32 const delta_time)
{
	...
	float const	air_resistance	= (GameID() == GAME_SINGLE) ? m_fAirResistanceK : bullet.air_resistance;
	bullet.tracer_start_position= bullet.bullet_pos;
...
void CBulletManager::Render	()
{
	...
	FVF::LIT	*verts		=	(FVF::LIT	*) RCache.Vertex.Lock((u32)bullet_num*12,
	...
	for(BulletVecIt it = m_BulletsRendered.begin(); it!=m_BulletsRendered.end(); it++){
		...
		tracers.Render			(verts, bullet->bullet_pos, center, tracer_direction, length, width, bullet->m_u8ColorID, bullet->speed, bActor);
	}
