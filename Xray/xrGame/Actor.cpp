...
void CActor::g_Physics			(Fvector& _accel, float jump, float dt)
{
	...
	
	{
		...
		if (!fis_zero(health_lost))		
		{
			...
			{
				SHit HDS = SHit(health_lost,hdir,di->DamageInitiator(),character_physics_support()->movement()->ContactBone(),di->HitPos(),0.f,di->HitType(),0.f,b_hit_initiated);
				...
				{
					static HUD_SOUND_ITEM m_buzzHit;

					if (0 == m_buzzHit.sounds.size())
						HUD_SOUND_ITEM::LoadSound("zone_buzz", "hit_sound", m_buzzHit, SOUND_TYPE_ITEM);

					if ( !m_buzzHit.playing() )
						HUD_SOUND_ITEM::PlaySound(m_buzzHit, Position(), this, true);
				}
...
float	CActor::HitArtefactsOnBelt		(float hit_power, ALife::EHitType hit_type)
{
	...
	if (res_hit_power_k < 0.2f) res_hit_power_k = 0.2f;
