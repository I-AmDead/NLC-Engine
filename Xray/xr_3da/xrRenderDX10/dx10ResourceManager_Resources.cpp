...
SVS*	CResourceManager::_CreateVS		(LPCSTR _name)
{
	string_path			name;
	strcpy_s				(name,_name);
	if (0 == ::Render->m_skinning)	strcat_s(name,"_0");
	if (1 == ::Render->m_skinning)	strcat_s(name,"_1");
	if (2 == ::Render->m_skinning)	strcat_s(name,"_2");
	if (3 == ::Render->m_skinning)	strcat_s(name,"_3");
	if (4 == ::Render->m_skinning)	strcat_s(name,"_4");
	LPSTR N				= LPSTR		(name);
	...
	else
	{
		...
		{
			string1024			tmp;
			sprintf_s			(tmp, "error DX10: %s is missing. Replace with stub_default.vs", cname);
			Msg					(tmp);
...
SPS*	CResourceManager::_CreatePS			(LPCSTR _name)
{
	string_path			name;
	strcpy_s				(name,_name);
	if (0 == ::Render->m_MSAASample)	strcat_s(name,"_0");
	if (1 == ::Render->m_MSAASample)	strcat_s(name,"_1");
	if (2 == ::Render->m_MSAASample)	strcat_s(name,"_2");
	if (3 == ::Render->m_MSAASample)	strcat_s(name,"_3");
	if (4 == ::Render->m_MSAASample)	strcat_s(name,"_4");
	if (5 == ::Render->m_MSAASample)	strcat_s(name,"_5");
	if (6 == ::Render->m_MSAASample)	strcat_s(name,"_6");
	if (7 == ::Render->m_MSAASample)	strcat_s(name,"_7");
	LPSTR N				= LPSTR(name);
	...
	else
	{
		...
		if (!R)
		{
			...
			//Memory.mem_compact();
			sprintf_s				(tmp, "error DX10: %s is missing. Replace with stub_default.ps", cname);
			Msg					(tmp);
...
SGS*	CResourceManager::_CreateGS			(LPCSTR name)
{
	...
	else
	{
		...
		if (!R)
		{
			...
			//	TODO: HACK: Test failure
			//Memory.mem_compact();
			sprintf_s				(tmp, "error DX10: %s is missing. Replace with stub_default.gs", cname);
			Msg					(tmp);
...
CTexture* CResourceManager::_CreateTexture	(LPCSTR _Name)
{
	...
	if (0==xr_strcmp(_Name,""))
	{
		Msg	("! ERROR: Failed to find texture on LTX (_CreateTexture) '%s'", _Name);
		return 0;
	}
