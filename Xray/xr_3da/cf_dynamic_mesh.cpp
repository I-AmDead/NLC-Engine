...
BOOL	CCF_DynamicMesh::_RayQuery( const collide::ray_defs& Q, collide::rq_results& R)
{
	...
	VERIFY( owner );
	VERIFY( owner->Visual() );
	CKinematics *K = owner->Visual()->dcast_PKinematics();
	...
	struct spick
	{
		 ...
		 spick(  const collide::ray_defs& Q_, const CObject &obj_, CKinematics &K_ ): Q( Q_ ), obj( obj_ ), K( K_ )
		 {
	...
	} pick( (collide::ray_defs&) (Q), (const CObject &)(*owner),(CKinematics&) (*K) );
