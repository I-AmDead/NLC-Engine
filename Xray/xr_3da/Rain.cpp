...
static const float	source_radius = 25.f;// 12.5f;
...
ENGINE_API rain_timer_params *rain_timers_raycheck;
ENGINE_API rain_timer_params *rain_timers;
ENGINE_API Fvector4 rain_params;
...
//	if (factor<EPS_L)			return;
	...
	//rain params update
	{
		...
		rain_params.x = (rain_timers->timer.x - rain_timers->timer.y)/40.f/*ps_r2_rain_params.x*/
					+ lerp<float>(0, saturate(rain_timers->timer.y/40.f/*ps_r2_rain_params.x*/), saturate(rain_timers->timer.y));
		rain_params.y = (rain_timers->timer.x - rain_timers->timer.y)/30.f/*ps_r2_rain_params.y*/
					+ lerp<float>(0, saturate(rain_timers->timer.y/30.f/*ps_r2_rain_params.y*/), saturate(rain_timers->timer.y));
		rain_params.z = (rain_timers->timer.x - rain_timers->timer.y)/20.f/*ps_r2_rain_params.z*/
					+ lerp<float>(0, saturate(rain_timers->timer.y/20.f/*ps_r2_rain_params.z*/), saturate(rain_timers->timer.y));
		rain_params.w = factor;
	}

//	Msg("rain_params: %f,%f,%f,%f", wet_params.x, wet_params.y, wet_params.z, wet_params.w);
	if (NO_RAIN==rain_timers_raycheck->Update(0, true)) {
		rain_flag = false;
		return;
	}
	if (!rain_flag) {
