...
CRenderTarget::CRenderTarget		()
{
   ...
   if( RImplementation.o.dx10_msaa )
	...
	b_clear_hud					= xr_new<CBlender_ClearHUD>			();

	//	NORMAL
	{
		u32		w=Device.dwWidth, h=Device.dwHeight;
		rt_Position.create			(r2_RT_P,		w,h,D3DFMT_A16B16G16R16F, SampleCount );
//		rt_Scene.create			(r2_RT_scene,	w,h,D3DFMT_A8R8G8B8		, 1 );
//		rt_ScenePrev.create			(r2_RT_scene_prev,	w,h,D3DFMT_A8R8G8B8		, SampleCount );
//		rt_Reflect.create(r2_RT_reflect, w, h, D3DFMT_A8R8G8B8, SampleCount);
		rt_UI3dStatic.create		(r2_RT_3dstatic,		w, h,	D3DFMT_A8R8G8B8,	1);
		rt_UI3dStatic_prev0.create	(r2_RT_3dstatic_prev0,	w, h,	D3DFMT_A8R8G8B8,	1);
		rt_UI3dStatic_prev1.create	(r2_RT_3dstatic_prev1,	w, h,	D3DFMT_A8R8G8B8,	1);
		rt_UI3dStatic_prev2.create	(r2_RT_3dstatic_prev2,	w, h,	D3DFMT_A8R8G8B8,	1);
		rt_UI3dStatic_prev3.create	(r2_RT_3dstatic_prev3,	w, h,	D3DFMT_A8R8G8B8,	1);
		//rt_UI3dStatic_r.create		(r2_RT_3dstatic_r,		w, h,	D3DFMT_A8R8G8B8,	SampleCount);
		//rt_UI3dStatic_ZB.create		(r2_RT_3dstatic_ZB,		w, h,	D3DFMT_D24S8,		SampleCount);
		...
		rt_Generic_prev0.create		  (r2_RT_generic_prev0,	w,h,   D3DFMT_A8R8G8B8, 1		);
		rt_Generic_prev1.create		  (r2_RT_generic_prev1,	w,h,   D3DFMT_A8R8G8B8, 1		);

		rt_Hud_prev0.create		  (r2_RT_hud_prev0,	w,h,   D3DFMT_A8R8G8B8, 1		);
		rt_Hud_prev1.create		  (r2_RT_hud_prev1,	w,h,   D3DFMT_A8R8G8B8, 1		);
		//rt_Hud_ZB.create		  (r2_RT_hud_ZB,	w,h,   D3DFMT_D24S8,	1		);
	...
	s_clear_hud.create				(b_clear_hud,	"r2\\clear_hud");
	...
	// Build textures
	{
		...
		if (1)
		{
			...
			//	Create noise mipped
			{
				...
				t_noise_mipped = DEV->_CreateTexture(r2_jitter_mipped);
...
/*	if (1)
	{
		...
		texRTDesc.Width = Device.dwWidth;
		texRTDesc.Height = Device.dwHeight;
		texRTDesc.MipLevels = 1;
		texRTDesc.ArraySize = 4;
		texRTDesc.SampleDesc.Count = 1;
		texRTDesc.SampleDesc.Quality = 0;
		texRTDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
		texRTDesc.Usage = D3D11_USAGE_DEFAULT;
		texRTDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
		texRTDesc.CPUAccessFlags = 0;
		texRTDesc.MiscFlags = 0;
		ID3D11Texture2D* texRT;
		HRESULT hr = HW.pDevice->CreateTexture2D(&texRTDesc, 0, &texRT);
		R_ASSERT(hr == S_OK);

		D3D11_RENDER_TARGET_VIEW_DESC rtvRTDesc;
		rtvRTDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
		rtvRTDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2DARRAY;
		rtvRTDesc.Texture2DArray.MipSlice = 0;
		rtvRTDesc.Texture2DArray.ArraySize = 1;

		for (u32 i = 0; i < 4; ++i)
		{
			rtvRTDesc.Texture2DArray.FirstArraySlice = i;
			hr = HW.pDevice->CreateRenderTargetView(texRT, &rtvRTDesc, &rt_UI3dStatic_TAA[i]);
			R_ASSERT(hr == S_OK);
		}

		t_3dstatic_taa = DEV->_CreateTexture("$user$3dstatic_taa");
		t_3dstatic_taa->surface_set(texRT);
		_RELEASE(texRT);
	}*/
...
//	s_reflect.create					("scene_reflect");
...
//	_RELEASE					(rt_smap_ZB);
	...
	xr_delete					(b_clear_hud					);

/*	for(u32 i = 0; i<0; i++)
		_RELEASE(rt_UI3dStatic_TAA[i]);

	t_3dstatic_taa.destroy();*/
}
