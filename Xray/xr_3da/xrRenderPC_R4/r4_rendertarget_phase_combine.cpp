...
void	CRenderTarget::phase_combine	()
{
	...
	if( !RImplementation.o.dx10_msaa )
	{
		...
		if (Device.dwFrame & 1)
		{
			HW.pContext->ClearRenderTargetView(rt_Generic_prev0->pRT, ColorRGBA);
			u_setrt(rt_Generic_prev0, 0, 0, HW.pBaseZB);
		}
		else
		{
			HW.pContext->ClearRenderTargetView(rt_Generic_prev1->pRT, ColorRGBA);
			u_setrt(rt_Generic_prev1, 0, 0, HW.pBaseZB);
		}
	}
	else
	{
		...
		u_setrt				( rt_Generic_0_r,0,0,RImplementation.Target->rt_MSAADepth->pZRT );
	}
	...
	if (!_menu_pp)
	{
		...
		Fvector4	envclr;
			if (!ps_r2_ls_flags.test(R2FLAG_HEMI_COP))
		...
		// Setup textures
		ID3DBaseTexture*	e0	= _menu_pp?0:envdesc.sky_r_textures_env[0].second->surface_get();
		ID3DBaseTexture*	e1	= _menu_pp?0:envdesc.sky_r_textures_env[1].second->surface_get();
...
/*
	{
		//HW.pContext->CopyResource(rt_ScenePrev->pTexture->surface_get(), rt_Scene->pTexture->surface_get());
		HW.pContext->ClearRenderTargetView(rt_Scene->pRT, ColorRGBA);

		if (!RImplementation.o.dx10_msaa)
			u_setrt(rt_Scene, 0, 0, HW.pBaseZB);
		else
			u_setrt(rt_Scene, 0, 0, RImplementation.Target->rt_MSAADepth->pZRT);
...
*/
	// Forward rendering
	{
		...
		//HW.pContext->ClearRenderTargetView(rt_Reflect->pRT, ColorRGBA);
/*
		if (!RImplementation.o.dx10_msaa)
		{
			if (Device.dwFrame & 1)
...
*/
   ...
   if( RImplementation.o.dx10_msaa )
   {
		...
		if (Device.dwFrame & 1)
			HW.pContext->ResolveSubresource( rt_Generic_prev0->pTexture->surface_get(), 0, rt_Generic_0_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM );
		else
			HW.pContext->ResolveSubresource( rt_Generic_prev1->pTexture->surface_get(), 0, rt_Generic_0_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM );
   }
   ...
   // for msaa we need a resolved color buffer - Holger
	...
	BOOL	bDistort	= RImplementation.o.distortion_enabled;				// This can be modified
	{
		...
		if (bDistort)		
		{
			...
			if( !RImplementation.o.dx10_msaa )
			{
				u_setrt(rt_Color,0,0,HW.pBaseZB);		// Now RT is a distortion mask
			}
			else
			{
				u_setrt(rt_Color,0,0,RImplementation.Target->rt_MSAADepth->pZRT);		// Now RT is a distortion mask
			}
			RCache.set_CullMode			(CULL_CCW);
			RCache.set_Stencil			(FALSE);
			RCache.set_ColorWriteEnable	();
...
/*
   ...
   */
	...
	//HW.pContext->ClearRenderTargetView(rt_Generic->pRT, ColorRGBA);
//	if ( Device.dwFrame & 1 )
//		u_setrt		( rt_Generic_prev0,0,0,HW.pBaseZB );			// LDR RT
//	else
//		u_setrt		( rt_Generic_prev1,0,0,HW.pBaseZB );
...
//.		RCache.set_c				("dof_params",	ps_r2_dof.x, ps_r2_dof.y, ps_r2_dof.z, ps_r2_dof_sky);
	...
	//HW.pContext->CopyResource(rt_Generic->pTexture->surface_get(), rt_Generic_0->pTexture->surface_get());
	//HW.pContext->ClearRenderTargetView(rt_Scene->pRT, ColorRGBA);
//	ID3D11Resource *pSrc;
//	HW.pBaseRT->GetResource(&pSrc);
//	HW.pContext->CopyResource(rt_Scene->pTexture->surface_get(), pSrc);
//	_RELEASE(pSrc);
...
/*
void CRenderTarget::phase_reflect	()
{
	u32			Offset = 0;
...

	{
		...
		// Draw
		RCache.set_Element			(s_reflect->E[0]	);
		RCache.set_Geometry			(g_combine		);
...
void CRenderTarget::phase_taa()
{
...

	{
		...
		// Draw
		RCache.set_Element			(s_clear_hud->E[5]	);
		RCache.set_Geometry			(g_combine		);

		RCache.set_c				("iPhaseTAA", (Device.dwFrame & 1) ? 0 : 1);

		RCache.Render				(D3DPT_TRIANGLELIST,Offset,0,4,0,2);
	}
}

void CRenderTarget::phase_combine_hud()
{
	bool	_menu_pp = g_pGamePersistent ? g_pGamePersistent->OnRenderPPUI_query() : false;
	if ( _menu_pp || !g_pGameLevel) return;
	...
	if (!RImplementation.o.dx10_msaa)
	{
		HW.pContext->ClearDepthStencilView(HW.pBaseZB, D3D11_CLEAR_STENCIL, 1.0f, 0);
		u_setrt(rt_Position, rt_Color, HW.pBaseZB);
	}
	else
	{
		HW.pContext->ClearDepthStencilView(rt_MSAADepth->pZRT, D3D11_CLEAR_STENCIL, 1.0f, 0);
		u_setrt(rt_Position, rt_Color, rt_MSAADepth->pZRT);
	}
	//HW.pContext->ClearRenderTargetView(rt_Position->pRT, ColorRGBA);
	RImplementation.r_dsgraph_render_hud(0);

	if (!RImplementation.o.dx10_msaa)
	{
		if (Device.dwFrame & 1)
		{
			HW.pContext->ClearRenderTargetView(rt_Hud_prev0->pRT, ColorRGBA);
			u_setrt(rt_Hud_prev0, 0, 0, HW.pBaseZB);
		}
		else
		{
			HW.pContext->ClearRenderTargetView(rt_Hud_prev1->pRT, ColorRGBA);
			u_setrt(rt_Hud_prev1, 0, 0, HW.pBaseZB);
		}
	}
	else
	{
	...
	RImplementation.r_dsgraph_render_hud(1);

	if (RImplementation.o.dx10_msaa)
	{
		if (Device.dwFrame & 1)
			HW.pContext->ResolveSubresource(rt_Hud_prev0->pTexture->surface_get(), 0, rt_Generic_0_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM);
		else
			HW.pContext->ResolveSubresource(rt_Hud_prev1->pTexture->surface_get(), 0, rt_Generic_0_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM);
	}

	//		if (Device.dwFrame & 1)
	//			HW.pContext->CopyResource(rt_Hud_prev0->pTexture->surface_get(), rt_Generic_0->pTexture->surface_get());
	//		else
	//			HW.pContext->CopyResource(rt_Hud_prev1->pTexture->surface_get(), rt_Generic_0->pTexture->surface_get());
}