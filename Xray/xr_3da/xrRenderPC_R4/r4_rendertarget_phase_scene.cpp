...
void	CRenderTarget::phase_scene_prepare	()
{
	...
	if ( RImplementation.o.advancedpp &&
			(
				ps_r2_ls_flags.test(R2FLAG_SOFT_PARTICLES|R2FLAG_DOF|R2FLAG_REFLECTIONS) ||
...
void	CRenderTarget::phase_clear_hud()
{
	u8 uPhase = g_pGamePersistent && g_pGamePersistent->ZoomRotationFactor() > 0.65f ? 0 : 1;
...

	{
		...
		// Draw
		RCache.set_Element			(s_clear_hud->E[uPhase]	);
		RCache.set_Geometry			(g_combine		);
