...
#include "../xrRenderDX10/ShaderResourceTraits.h"
...
static class cl_pos_decompress_params_hud		: public R_constant_setup		{	virtual void setup	(R_constant* C)
{
	float VertTan =  -1.0f * tanf( deg2rad(psHUD_FOV*Device.fFOV/2.0f ) );
	float HorzTan =  - VertTan / Device.fASPECT;
...
{	
	virtual void setup	(R_constant* C)
	{
		...
		RCache.set_c	(C, fValue, VIEWPORT_NEAR, E.far_plane, 0);
	}
...
extern Fmatrix m_previous;
static class cl_proj_prev	: public R_constant_setup 
{	
	virtual void setup (R_constant* C) 
	{ 
...
extern Fmatrix m_previous_hud;
static class cl_proj_prev_hud	: public R_constant_setup 
{	
	virtual void setup (R_constant* C) 
	{ 
		RCache.set_c(C, m_previous_hud);
	}
}	binder_proj_prev_hud;
/*
static class cl_3d_static_light_dir : public R_constant_setup		
{	
	virtual void setup	(R_constant* C)
	{
		RCache.set_c	(C, ps_r1_3d_static_light_dir.x, ps_r1_3d_static_light_dir.y, ps_r1_3d_static_light_dir.z, 1.f);
	}
}	binder_3d_static_light_dir;
*/
...
void					CRender::create					()
{
	...
	*/
	...
	DEV->RegisterConstantSetup	("water_intensity",	&binder_water_intensity);
	DEV->RegisterConstantSetup	("sun_shafts_intensity",	&binder_sun_shafts_intensity);
	DEV->RegisterConstantSetup	("m_AlphaRef",	&binder_alpha_ref);
	DEV->RegisterConstantSetup	("pos_decompression_params",	&binder_pos_decompress_params);
	DEV->RegisterConstantSetup	("pos_decompression_params_hud",	&binder_pos_decompress_params_hud);
	DEV->RegisterConstantSetup	("pos_decompression_params2",	&binder_pos_decompress_params2);
	DEV->RegisterConstantSetup	("triLOD",	&binder_LOD);
	DEV->RegisterConstantSetup	("mProjPrev",	&binder_proj_prev);
	DEV->RegisterConstantSetup	("mProjPrevHUD",	&binder_proj_prev_hud);
	//DEV->RegisterConstantSetup	("m_3d_static_light_dir",	&binder_3d_static_light_dir);
...
void					CRender::flush_3d_static		()					{ /*r_dsgraph_render_3d_static	()*/b_render_3d_static = true;						}
...
void CRender::set_taa_project_matrix()
{
	if ( Device.dwFrame & 1 )
	{
		Device.mProject._31 += 0.5f / Device.dwWidth;
		Device.mProject._32 += 0.5f / Device.dwHeight;
	}
	else
	{
		Device.mProject._31 -= 0.5f / Device.dwWidth;
	...
	void*&							result)
{
	...
	// options
	{
		...
		strcat_s(sh_name, c_smapsize); len+=4;
	}
	...
	if (RImplementation.o.advancedpp && ps_r2_ls_flags.test(R2FLAG_REFLECTIONS))
	{
   ...
   sh_name[len] = 0;
	...
	if ( !match_shader_id(name, sh_name, m_file_set, temp_file_name) ) {
		string_path file;
		strcpy_s		( file, "shaders_cache\\r4\\" );
		strcat_s		( file, name );
		strcat_s		( file, "." );
