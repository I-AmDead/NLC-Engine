...
void CRender::ScreenshotImpl	(ScreenshotMode mode, LPCSTR name, CMemoryWriter* memory_writer)
{
	...
	if (!ps_r2_ls_flags.test(R1FLAG_3DSTATIC_CREATE))
		HW.pBaseRT->GetResource(&pSrcTexture);
	else
	{
		Target->rt_UI3dStatic->pRT->GetResource(&pSrcTexture);
	}
		...
		case IRender_interface::SM_NORMAL:
			{
				...
				if (strstr(Core.Params,"-ss_bmp") || ps_r2_ls_flags.test(R1FLAG_3DSTATIC_CREATE))
				{ // hq
					sprintf_s			(buf,sizeof(buf),"ssq_%s_%s_(%s).bmp",Core.UserName,timestamp(t_stemp),(g_pGameLevel)?g_pGameLevel->name().c_str():"mainmenu");
					ID3DBlob*		saved	= 0;
#ifdef USE_DX11
