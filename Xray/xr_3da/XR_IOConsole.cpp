...
void CConsole::OnScreenRatioChanged()
{
	xr_delete( pFont );
	xr_delete( pFont2 );
}
...
void CConsole::OnRender()
{
	...
	if ( !m_hGeom_back )
	{
		m_hGeom_back.create(FVF::F_TL, RCache.Vertex.Buffer(), 0);
	}
...
void CConsole::DrawBackgrounds( bool bGame )
{
	...
	RCache.set_Shader(m_hShader_back);
...
//	UIRender->StartPrimitive( 6 + 12 + (VIEW_TIPS_COUNT+1)*6 + 12, IUIRender::ptTriList, IUIRender::pttTL );
	...
	FVF::TL* start_pv = (FVF::TL*)RCache.Vertex.Lock(6 + 12 + (VIEW_TIPS_COUNT+1)*6 + 12, m_hGeom_back.stride(), vOffset);
	FVF::TL* pv = start_pv;

	DrawRect( r, back_color, pv );
	...
	if ( m_tips.size() == 0 || m_disable_tips )
	{
		...
		RCache.Vertex.Unlock		(u32(p_cnt),m_hGeom_back.stride());
		RCache.set_Geometry	 		(m_hGeom_back);
		u32 primCount				= (u32)(p_cnt/3);
		if (primCount>0)			RCache.Render(D3DPT_TRIANGLELIST,vOffset,primCount);
	...
	DrawRect( pr, tips_back_color, pv );
	DrawRect( sr, tips_select_color, pv );
	...
	if ( tips_sz > VIEW_TIPS_COUNT )
	{
		...
		rb.x2 = rb.x1 + 2 * w1;
		rb.y2 = pr.y2;
		DrawRect( rb, tips_scroll_back_color, pv );
		...
		DrawRect( rs, tips_scroll_pos_color, pv );
	}
...
void CConsole::DrawRect( Frect const& r, u32 color, FVF::TL*& pv )
{
	pv->set( r.x1, r.y1, color, 0.0f, 0.0f ); pv++;
	pv->set( r.x2, r.y1, color, 1.0f, 0.0f ); pv++;
	pv->set( r.x2, r.y2, color, 1.0f, 1.0f ); pv++;

	pv->set( r.x1, r.y1, color, 0.0f, 0.0f ); pv++;
	pv->set( r.x2, r.y2, color, 1.0f, 1.0f ); pv++;
	pv->set( r.x1, r.y2, color, 0.0f, 1.0f ); pv++;
}
