...
public:
	...
	IC void r_seek	(u32 pos, bool _check=true)
	{
		VERIFY		(pos < B.count);
		if (_check && r_pos >= B.count)
			//LogPacketError("!#ERROR: NET_PACKET::r_seek() buffer overrun, r_pos = %d , size = %d", r_pos, B.count);
			Msg("!#ERROR: NET_PACKET::r_seek() buffer overrun, r_pos = %d , size = %d", r_pos, B.count);
		r_pos		= pos;
	}
	...
	IC void		r				( void* p, u32 count)
	{
		...
		if (r_pos > B.count)
			...
			Msg("!#ERROR: NET_PACKET::r() buffer overrun, r_pos = %d , size = %d", r_pos, B.count);
	...
	IC void		r_advance		(u32 size)
	{
		...
		if (r_pos > B.count)
			...
			Msg("!#ERROR:  NET_PACKET::r_advance() buffer overrun, r_pos = %d , size = %d", r_pos, B.count);
